...
import {
  ...
  Shield, Bell, BookOpen, Clock, Lock, AlertCircle, CheckCircle
} from 'lucide-react';
import axios from 'axios';
import { useAuth } from '../context/AuthContext';

type Tab = 'profile' | 'teaching' | 'security' | 'notifications';
...
const LecturerSettings: React.FC = () => {
  ...
  useEffect(() => {
    if (!token) return;
    axios.get('http://localhost:5000/api/lecturer/profile', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => {
        const d = r.data;
        setProfile({
  ...
  const saveProfile = async () => {
    ...
    try {
      await axios.put('http://localhost:5000/api/lecturer/profile', profile, {
  ...
  const changePassword = async () => {
    ...
    try {
      await axios.post(
        'http://localhost:5000/api/lecturer/change-password',
  ...
  const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'teaching',      label: 'Teaching',  icon: <BookOpen size={16} /> },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Manage your profile, teaching details, and preferences</p>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 space-y-6">
              ...
              <div className="flex items-center gap-6">
                <div className="relative flex-shrink-0">
                  <div className="h-24 w-24 rounded-full overflow-hidden bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center ring-4 ring-indigo-50 dark:ring-indigo-900/20">
                    {profile.avatar
                  ...
                  <button
                    ...
                    className="absolute bottom-0 right-0 bg-indigo-600 text-white rounded-full p-1.5 shadow-lg hover:bg-indigo-700 transition"
                  >
                    <Camera size={14} />
                  </button>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <button onClick={() => fileRef.current?.click()} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
                      Change photo
                    </button>
                    {profile.avatar && (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  maxLength={200}
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition resize-none"
          ...
          {tab === 'teaching' && (
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 space-y-6">
              <div>
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">Teaching Details</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">Add your title and availability so students know how to reach you.</p>
              </div>

              {/* Title prefix */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Title / Honorific</label>
                ...
                {profile.title && (
                  <p className="text-xs text-gray-400 mt-2">
                    Display name: <span className="font-semibold text-gray-700 dark:text-gray-300">{profile.title} {profile.full_name || 'Your Name'}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <Clock size={14} className="text-gray-400" /> Office Hours
                  </span>
                </label>
                <textarea
                  value={profile.office_hours}
                  onChange={e => setProfile(p => ({ ...p, office_hours: e.target.value }))}
                  rows={3}
                  maxLength={300}
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition resize-none"
                  placeholder="e.g. Monday & Wednesday 2–4pm, or by appointment via email"
                />
                <p className="text-xs text-gray-400 mt-1 text-right">{profile.office_hours.length}/300</p>
              </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 space-y-6">
                ...
                <div key={field}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-4 py-2.5 pr-12 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                      placeholder={
              ...
              <div className="flex justify-end">
                <button
                  ...
                  disabled={pwSaving}
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-semibold text-sm bg-indigo-600 hover:bg-indigo-700 text-white transition disabled:opacity-60"
                >
                  <Shield size={16} />
          ...
          {tab === 'notifications' && (
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-8 space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Control how you receive student alerts and messages.</p>
              </div>
              ...
              {[
                {
                  ...
                  desc: 'Receive student alerts, group updates, and messages via email',
                  icon: <Mail size={20} className="text-indigo-500" />,
                },
                {
                  key: 'notify_inapp' as const,
                  label: 'In-App Notifications',
                  desc: 'See at-risk alerts and updates inside the dashboard',
                ...
                <div key={item.key} className="flex items-center justify-between p-5 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 transition">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 ml-4 ${
                      profile[item.key] ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-gray-700'
                    }`}
                  >
